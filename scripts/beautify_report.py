...
def collect_table_ns_gray_requests_from_blocks(
    ...
    """
    Gray out placeholders in non-significant rows.

    Rendering only:
    - row significance comes from the relative-change cell's written marker/text
    - conservative fallback: a bare dash row is treated as ns only when the row also shows
      explicit non-significant hints (`不显著` or dashed p-value cell)
    """
    requests: list[dict[str, Any]] = []
    for tb in all_blocks:
        if tb.get("block_type") != BLOCK_TABLE:
            ...
            if sig is None and rel_text in ("—", "\u2014"):
                ...
                for j in range(n_cols):
                    ...
                    if txt.startswith("➖ ") or txt == "不显著":
                        saw_ns_hint = True
                        break
                if not saw_ns_hint and p_col is not None:
                    cid = cell_ids[r * n_cols + p_col]
